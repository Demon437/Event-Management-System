...
const Navbar = () => {
  ...
  return (
    <motion.nav
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'glass-dark shadow-2xl' : 'bg-transparent'
        }`}
    >
        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-1">
            ...
            {isAuthenticated ? (
              ...
              <Link
                to="/login"
                className={`ml-4 px-4 py-2 rounded-lg font-medium transition-all duration-300 ${location.pathname === '/login'
                    ? 'bg-gradient-to-r from-primary-600 to-accent-600 text-white'
                    : 'text-gray-300 hover:text-white hover:bg-white/10'
                  }`}
              >
          ...
          <div className="px-4 py-4 space-y-2">
            ...
            {isAuthenticated ? (
              ...
              <Link
                to="/login"
                onClick={() => setIsOpen(false)}
                className={`block px-4 py-3 rounded-lg font-medium transition-all ${location.pathname === '/login'
                    ? 'bg-gradient-to-r from-primary-600 to-accent-600 text-white'
                    : 'text-gray-300 hover:bg-white/10'
                  }`}
              >
